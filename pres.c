...
/* NEW: presence flags so we don't hang on missing devices */
uint8_t gIna1Present = 0;
uint8_t gIna2Present = 0;
uint8_t gOledPresent = 0;
...
/* Task 1: Sample INA219s and print detailed live readings */
void Task_SampleINA(void) {
  ...
  /* NEW: verbose serial debug */
  char msg[196];
  snprintf(msg, sizeof(msg),
           "CH1: %5.2f V %7.3f mA %6.3f W | "
           "CH2: %5.2f V %7.3f mA %6.3f W | "
           "Tot: %6.3f W  %6.3f Wh\r\n",
           ch1.v_V, ch1.i_A * 1000.0f, ch1.p_W,
           ch2.v_V, ch2.i_A * 1000.0f, ch2.p_W,
           totalW, totalWh);
  debug_print(msg);
}
...
int main(void)
{
  ...
  /* Probe devices so we don't hang on ones that are missing */
  if (HAL_I2C_IsDeviceReady(&hi2c1, INA219_ADDR1, 2, 10) == HAL_OK) {
    INA_Init(INA219_ADDR1);
    gIna1Present = 1;
    debug_println("INA219 #1 detected and initialized.");
  } else {
    debug_println("WARNING: INA219 #1 NOT detected, skipping.");
  }

  if (HAL_I2C_IsDeviceReady(&hi2c1, INA219_ADDR2, 2, 10) == HAL_OK) {
    INA_Init(INA219_ADDR2);
    gIna2Present = 1;
    debug_println("INA219 #2 detected and initialized.");
  } else {
    debug_println("WARNING: INA219 #2 NOT detected, skipping.");
  }

  if (HAL_I2C_IsDeviceReady(&hi2c1, OLED_ADDR, 2, 10) == HAL_OK) {
    gOledPresent = 1;
    OLED_Init();
    OLED_ClearBuffer();
    OLED_Update();
    debug_println("OLED detected and initialized.");
  } else {
    debug_println("WARNING: OLED NOT detected, skipping OLED init.");
  }

  //ESP_StartAccessPoint();   // keep commented for clean UART debug
  ...
  debug_println("Init complete, entering scheduler loop...");
  /* USER CODE END 2 */
