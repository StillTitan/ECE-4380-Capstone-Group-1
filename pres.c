...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Smart Apartment Energy Monitor (RTOS, INA current debug)
  *
  * This version:
  *  - Keeps RTOS, OLED, and ESP code in place.
  *  - DOES NOT start the ESP Wi-Fi AP (ESP_StartAccessPoint is commented out).
  *  - Periodically reads 2x INA219 sensors.
  *  - Prints live current readings (mA) for CH1 and CH2 over UART2.
  *  - RTOS scheduler runs 2 tasks:
  *      - Task_SampleINA (every 100 ms)
  *      - Task_Status (every 1000 ms)
  *
  * Expected serial (115200 8N1):
  *    Smart Apartment Energy Monitor (RTOS + AP) Boot
  *    Scanning I2C bus...
  *    - Found device at 0x40
  *    - Found device at 0x41
  *    - Found device at 0x3C
  *    I2C scan complete.
  *
  *    CH1: 250.000 mA   CH2:   0.000 mA
  *    CH1: 249.500 mA   CH2: 500.200 mA
  *    ...
  *    Status tick: 0
  *    Status tick: 1
  *    ...
...
typedef struct {
  ...
  float p_W_filt;  // filtered power (not used in this debug version)
  float E_Wh;      // accumulated energy (not used in this debug version)
} ChannelData_t;

/* Sample structure (kept for future use; not required for current-only debug) */
typedef struct {
  float ch1_W;
  float ch2_W;
  float total_W;
...
/* === OLED (SSD1306) definitions (kept for later use) === */
#define OLED_ADDR   (0x3C << 1)
#define OLED_WIDTH  128
#define OLED_HEIGHT 64
#define OLED_PAGES  (OLED_HEIGHT / 8)

/* Smoothing factor (not used in this debug version) */
static const float alpha = 0.2f;

/* === ESP-01S Access Point configuration (kept, but NOT used) === */
...
/* Simple queue kept for future use (OLED, etc.) */
typedef struct {
...
/* OLED framebuffer (not required for current-only debug) */
...
/* OLED helpers (kept for later use) */
...
/* ESP-01S helpers (kept, but NOT called) */
...
/* (Task_UpdateOLED exists but is unused in this debug-focused build) */
...
/* === Minimal 5x7 glyphs for digits and a few letters (kept for OLED) === */
...
/* === OLED (not used in this debug build, but left intact) === */
void OLED_Cmd(uint8_t cmd) {
...
/* === ESP-01S: Soft Access Point (kept, but not used) === */
...
int RTOS_Queue_Pop(EnergySample_t *s) {
  if (gEnergyQueue.count == 0) return -1;
...
/* SysTick hook: called by HAL every 1 ms */
...
/* Task_SampleINA:
 *  - Reads both INA219s
 *  - Prints live current (mA) over UART
 *  - No smoothing or Wh integration in this debug version
 */
void Task_SampleINA(void) {
  float v, i, p;
  ...
  /* Live current debug print (mA) */
  char msg[128];
  snprintf(msg, sizeof(msg),
           "CH1: %7.3f mA   CH2: %7.3f mA\r\n",
           ch1.i_A * 1000.0f,
           ch2.i_A * 1000.0f);
  debug_print(msg);

  /* If you want to re-enable power/Wh calcs later, add them here. */
}

/* OLED task (unused right now, but kept for future use) */
void Task_UpdateOLED(void) {
  (void)RTOS_Queue_Pop; // silence unused warning if needed
  // For now, do nothing. You can restore the previous OLED logic later.
}
...
int main(void)
{
  ...
  // OLED_Init();           // not needed for current-only debug
  // OLED_ClearBuffer();
  // OLED_Update();

  /* IMPORTANT: Wi-Fi/ESP disabled for now so UART is clean for debug */
  // ESP_StartAccessPoint();

  /* Initialize RTOS and register tasks */
  RTOS_Init();
  RTOS_AddPeriodicTask(Task_SampleINA, 100);   // every 100 ms
  // RTOS_AddPeriodicTask(Task_UpdateOLED, 200);  // not used right now
  RTOS_AddPeriodicTask(Task_Status,    1000);  // every 1 s
  ...
  while (1)
  {
    RTOS_RunScheduler();  // Run ready tasks
    HAL_Delay(1);         // small idle
    /* USER CODE END WHILE */
